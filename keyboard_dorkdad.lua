...
function invert_selection()
  local unselected = filter(G.hand.cards, function(x)
    return -1
      == indexOf(G.hand.highlighted, function(y)
        return y == x
      end)
  end)
...
function select_hand(cards)
  ...
  for k, v in pairs(cards) do
    if
      indexOf(G.hand.highlighted, function(x)
        return x == v
      end) == -1
    then
...
function select_by_rank(rank)
  for i = 1, #G.hand.cards do
    card = G.hand.cards[i]
    if
      get_visible_rank(card) == rank
      and not has_value(G.hand.highlighted, card)
    then
      G.hand:add_to_highlighted(card)
    end
  end
...
function flush(suit)
  ...
  elseif four_fingers and #cards >= 4 then
    ...
    for i = #G.hand.cards, 1, -1 do
      card = G.hand.cards[i]
      if
        get_visible_suit(card) ~= suit
        and get_visible_suit(card) ~= "Wild"
      then
        table.insert(cards, card)
      end
    end
